...
from .sources import source_regions
from .surface import CartesianPatch, DualSurface


def prepare(root, destination):
    root, destination = Path(root), Path(destination)
    run = root / "artifacts/dual-formation-2026-09-23/dt000125"
    coarse = root / "artifacts/dual-formation-2026-09-23/dt00025"
    ...
    target_config = json.loads(
        (root / "artifacts/dual-cartesian-2026-09-22/sources-112/config.json").read_text()
    )["cases"][0]["faces"]
    target = np.array(
