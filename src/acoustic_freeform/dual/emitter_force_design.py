...
from .stigmatic import pair_prescription
...
def run(configuration, output):
    ...
    cache = (
        checked_cache(inputs, Path(settings["cache_from"]), output)
    ...
    if settings.get("coupled_linearization_file"):
        path = Path(settings["coupled_linearization_file"])
    ...
    for trial in range(settings["starts"]):
        ...
        if trial == 0 and settings.get("initial_source_file"):
            initial_path = Path(settings["initial_source_file"])
            source = np.repeat(
        ...
        if settings.get("optimizer") == "conic":
            from .conic_precision import optimize
