...
from .screening import PrecisionObjective, WhitenedObjective
...
def run(configuration, output):
    ...
    seed_path = data_path(settings["source_file"])
    ...
    if settings.get("reference_linearization_file"):
        path = data_path(settings["reference_linearization_file"])
    ...
    for iteration in range(settings["outer_iterations"]):
        ...
        if iteration == 0 and settings.get("initial_operator"):
            cache_path = data_path(settings["initial_operator"])
