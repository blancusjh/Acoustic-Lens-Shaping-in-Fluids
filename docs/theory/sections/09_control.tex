...
Use the geometric error $J_s$ defined in \cref{eq:shape-cost} on a general
target patch. Let $J_{\rm obs}$ be the declared dimensionless observation
objective, such as error in measured shape or flow. Cartesian optical
objectives will specialize it in \cref{sec:optics}; they are not needed to
define the shaping problem. For large departures use a declared geometric
distance and correspondence, rather than implicitly registering a target to
the evolving state.

Let $\bm S_a$ be an invertible scaling operator on the commanded fields
(for a finite array, a diagonal matrix of positive channel scales)
...
The norms are the declared Hilbert norms of the scaled spatial command
fields, integrated over their source supports; they reduce to Euclidean norms
only after discretization. The penalties describe command effort and smoothness; they are not
...
\begin{equation}
 ...
 \nabla_{u}j=(J_{\rm stat})_{u}
                       +\mathcal R_u^{\ast}\bm\lambda.
 \label{eq:stationary-adjoint}
\end{equation}
Here $j(u)=J_{\rm stat}(\bm y(u),u)$ is the reduced objective.
The derivatives include all state-dependent loads and boundary terms.
For the fixed acoustic source subproblem, the plus-sign residual convention
uses the negative of the field $Z$ in \cref{sec:source-adjoint}; both give
the same reduced gradient. The adjoint approach supplies many control derivatives from a state solve and
...
terminal constraints add multipliers. The explicit example in \cref{sec:source-adjoint} supplies those spatial
conditions for one wave subproblem. Spatial integrations by parts provide
the remaining adjoint boundary and interface conditions; these depend on the actual physical
...
multipliers and complementarity conditions. For a convex admissible history
set $\mathfrak A_{\rm ad}$ after those constraints are treated, first-order optimality
requires
\begin{equation}
 \int_0^{T_f}\inner{\bm g_a}{\bm v-\bm a}\dd t\geq0
       \quad\text{for every }\bm v\in\mathfrak A_{\rm ad}.
