...
from acoustic_freeform.dual.linear import CondensedFactor, ReusedCondensedFactor
...
def test_failed_iterative_solve_refreshes_the_factor(monkeypatch):
    ...
    monkeypatch.setattr(
        "acoustic_freeform.dual.linear.gmres", lambda matrix, rhs, **kwargs: (np.zeros_like(rhs), 1)
    )
